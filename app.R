...
server <- function(input, output, session) {
  ...
  output$network <- renderVisNetwork({
    ...
    cat("Rendering network, maxnodes:", maxnodes, ", storage$focus:", storage$focus, "\n")
    
    nodes_df <- NULL
    edges_df <- NULL
    
    if (rv$useCaseActive && !is.null(storage$subnet)) {
      cat("Rendering use-case network, nodes:", nrow(storage$nodes), "\n")
      ...
      if (is.null(act_trait)) {
        ...
        showNotification("No active trait selected. Please select a trait, node, or table row and click 'Focus on'.", type = "error")
        return(NULL)
      }
      ...
      tryCatch({
        subnet <- neighbor_nodes %>% 
          maxneighbors(myValues$fullnet, limit = maxnodes) %>% 
          nodes2network(myValues$fullnet)
        cat("Subnet nodes before limit:", nrow(subnet$nodes), "Edges:", nrow(subnet$edges), "\n")
        
        if (nrow(subnet$nodes) > maxnodes) {
          cat("Subnet exceeds maxnodes, trimming to:", maxnodes, "\n")
          focus_id <- myValues$fullnet$nodes$id[
            tolower(myValues$fullnet$nodes$TRAITID) == tolower(act_trait)
          ]
          if (length(focus_id) == 0) {
            stop("Focus node ID not found for TRAITID:", act_trait)
          }
          node_scores <- subnet$nodes %>%
            mutate(degree = sapply(id, function(x) {
              sum(myValues$fullnet$edges$from_id == x | myValues$fullnet$edges$to_id == x, na.rm = TRUE)
            })) %>%
            arrange(desc(degree))
          keep_nodes <- c(focus_id, head(node_scores$id[node_scores$id != focus_id], maxnodes - 1))
          subnet$nodes <- subnet$nodes %>% filter(id %in% keep_nodes)
          subnet$edges <- subnet$edges %>% filter(from_id %in% keep_nodes & to_id %in% keep_nodes)
        }
        
        cat("Final subnet nodes:", nrow(subnet$nodes), "Edges:", nrow(subnet$edges), "\n")
        storage$subnet <- subnet
        
        edge_cols <- tolower(names(subnet$edges))
        node_cols <- tolower(names(subnet$nodes))
        from_col <- if ("from_id" %in% edge_cols) "from_id" else stop("No 'from_id' in subnet$edges columns")
        to_col <- if ("to_id" %in% edge_cols) "to_id" else stop("No 'to_id' in subnet$edges columns")
        id_col <- if ("id" %in% node_cols) "id" else stop("No 'id' in subnet$nodes columns")
        
        edges_df <- data.frame(
          from = subnet$edges[[from_col]],
          to = subnet$edges[[to_col]],
          title = paste(subnet$edges[[from_col]], "->", subnet$edges[[to_col]]),
          color = ifelse(subnet$edges$sign > 0, "blue", "red"),
          width = 1,
          stringsAsFactors = FALSE
        )
        ...
        cat("Valid nodes:", length(valid_nodes), "IDs:", paste(head(valid_nodes, 5), collapse = ", "), "\n")
        invalid_edges <- !edges_df$from %in% valid_nodes | (!is.na(edges_df$to) & !edges_df$to %in% valid_nodes)
        ...
        if (nrow(edges_df) == 0) {
          ...
          showNotification("No valid edges found for the network.", type = "warning")
        }
        ...
        if ("color" %in% node_cols) nodes_df$color <- subnet$nodes$color
        if ("shape" %in% node_cols) nodes_df$shape <- subnet$nodes$shape
        ...
        cat("Error in network rendering for TRAITID:", act_trait, ", Error:", e$message, "\n")
        showNotification(paste("Error rendering for", act_trait, ":", e$message), type = "error")
        return(NULL)
      })
    }
    
    if (is.null(nodes_df) || is.null(edges_df) || nrow(edges_df) == 0) {
      cat("No valid nodes_df or edges_df, returning NULL\n")
      showNotification("Cannot render network: no valid nodes or edges.", type = "error")
      return(NULL)
    }
    ...
    visNetwork(nodes_df, edges_df, height = "800px", width = "100%") %>%
      ...
      visEvents(
        ...
        Shiny.onInputChange('current_node_selection', params.nodes);
      }",
      ...
      click = "function(params) {
        ...
        Shiny.onInputChange('click', params.nodes[0]);
      }"
      ) %>%
